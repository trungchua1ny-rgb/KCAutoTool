...
Version `2.20.2` adds Chapter 4 continuity chains, treats Ingredients/Thành phần as Flow's default rather than a required popup tab, and identifies duration tabs by stable `trigger/content-4/6/8` suffixes instead of dynamic Radix ids. It includes project anchor images, last-frame extraction, explicit Frames selection, and exact 4/6/8-second video settings. It retains Phase 3a Beat & Chain Planning with editable chain metadata and also powers KC Auto Tool with production-queue heartbeats, reusable local graphic-style presets, completeness-checked scene prompts, stick-figure motion locks, automatic Image/Video mode switching in one Flow tab, project-asset reuse, and the Phase
