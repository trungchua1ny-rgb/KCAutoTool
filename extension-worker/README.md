...
Version `2.22.0` stabilizes Flow video settings. It confirms 16:9 by both the visible label and Flow's `LANDSCAPE` tab identity, reopens the settings popup between fields, retries aspect ratio and duration twice, and records detailed DOM diagnostics on failure. Debugger clicks now include visible mouse movement and settling delays. It retains the automatic natural-name character tagging and per-Flow-project character asset reuse added in 2.21.0: a library name seen at least twice is mapped to its canonical token; its local reference image is uploaded on the first tagged prompt, then selected from Flow assets on later prompts. It also includes Chapter 4 continuity chains, default Ingredients/Thành phần handling, stable duration selectors, project anchors, last-frame extraction, explicit Frames selection, and exact 4/6/8-second settings. It retains production-queue heartbeats, reusable graphic-style presets, completeness-checked prompts, stick-figure motion locks, and the Phase
