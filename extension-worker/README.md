...
Version `2.26.0` implements pacing-fix Part A. Phase 3b now writes motion prompts against explicit 4/6/8-second budgets, and the Flow worker recompiles every video prompt with the same duration-aware `PACING LOCK`, so saved Phase 3 prompts also benefit when regenerated. Primary motion must occupy at least 60% of runtime; anticipation plus final settle is capped at 0.6s/1.5s/2s for 4s/6s/8s clips, and 4-second clips begin their primary action immediately. Natural real-world speed is required unless the source explicitly supports a deliberate emotional beat. It retains 2.25.0's current `IMAGE`, `Bắt đầu`, and `Kết thúc` control support, exact Flow-library asset reuse with safe local-import fallback, bounded scene references, and Start-frame-only video generation.
