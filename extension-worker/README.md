...
Version `2.21.0` adds automatic natural-name character tagging and per-Flow-project character asset reuse. A library name seen at least twice is mapped to its canonical token; its local reference image is uploaded on the first tagged prompt, then selected from Flow assets on later prompts. The worker falls back to upload when the project changes, the image changes, or the cached asset disappears. It also includes Chapter 4 continuity chains, default Ingredients/Thành phần handling, stable duration selectors, project anchors, last-frame extraction, explicit Frames selection, and exact 4/6/8-second settings. It retains production-queue heartbeats, reusable graphic-style presets, completeness-checked prompts, stick-figure motion locks, and the Phase
