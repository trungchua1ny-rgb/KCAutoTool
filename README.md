...
failures. Extension version `2.37.0` snapshots and locks onto the newly-created Flow render card, retries clicking that exact card every three seconds until its viewer opens, then retries native `Tải xuống` every three seconds until Chrome creates the download. It waits for the file to finish before clicking `Xong`. Grid detection, direct signed HTTPS, and blob conversion remain fallbacks. It retains stable result identity, single-popup `VIDEO_FRAMES` / `VIDEO_REFERENCES` configuration, clean-composer handling, duration-aware pacing, bounded continuation references, and Start-frame-only video generation; reload
