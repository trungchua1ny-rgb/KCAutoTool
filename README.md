...
failures. Extension version `2.36.0` snapshots Flow result cards before submit and tracks the newly-created rendering card without clicking it. Only after that exact card gains a playable source and becomes enabled does the worker open it, click native `Tải xuống`, capture and name the Chrome download, then click `Xong`. Grid detection, direct signed HTTPS, and blob conversion remain fallbacks. It retains short recoverable polling, stable new-result confirmation, single-popup `VIDEO_FRAMES` / `VIDEO_REFERENCES` configuration, clean-composer handling, duration-aware pacing, current `Bắt đầu`/`Kết thúc` controls, bounded continuation references, and Start-frame-only video generation; reload
