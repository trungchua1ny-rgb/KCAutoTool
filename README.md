...
failures. Extension version `2.34.0` recognizes compact video cards in split-screen Flow windows, opens the newest completed card, clicks Flow's native `Tải xuống` action, captures and names that Chrome download for the active scene, then clicks `Xong`. Direct signed HTTPS and blob conversion remain fallbacks. It retains short recoverable video polling, a full hidden/visible baseline, stable new-result confirmation, single-popup `VIDEO_FRAMES` / `VIDEO_REFERENCES` configuration, clean-composer handling, duration-aware pacing, current `Bắt đầu`/`Kết thúc` controls, bounded continuation references, and Start-frame-only video generation; reload
