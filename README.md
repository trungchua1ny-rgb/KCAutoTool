...
failures. Extension version `2.38.0` waits ten seconds after video submit, then clicks the final media card in Flow's grid every two seconds until its viewer opens. After another five seconds it clicks native `Tải xuống` every two seconds until Chrome creates the download, waits for the file to finish, and clicks `Xong`. Direct signed HTTPS and blob conversion remain fallbacks. It retains stable result identity, single-popup `VIDEO_FRAMES` / `VIDEO_REFERENCES` configuration, clean-composer handling, duration-aware pacing, bounded continuation references, and Start-frame-only video generation; reload
